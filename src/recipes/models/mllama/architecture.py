...
from recipes.models.mllama.configuration_mllama import MllamaConfig, MllamaTextConfig, MllamaVisionConfig
...
@add_start_docstrings(
    """The Fast Mllama model which consists of a vision encoder and a language model.""",
    MLLAMA_START_DOCSTRING,
)
class FastMllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    _supports_quantized_cache = False  # quant cache not supported in encoder-decoder setting
    ...
    def forward(
        ...
        if cross_attention_states is not None:
            cross_attention_states = cross_attention_states.to(self.multi_modal_projector.weight.dtype)
            cross_attention_states = self.multi_modal_projector(cross_attention_states).reshape(
                -1, cross_attention_states.shape[-2], self.hidden_size
            )
