...
class Trainer:
    ...
    def after_train(self):
        ...
        # save ...
        metrics_filename = f"{self.train_config.output_dir}/train_metrics-{self.rank}-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.pth"
    ...
    def trace(self):
        self.profiler.step()
        if (self.step + 1) % len(self.train_dataloader) == 0 and self.rank == 0:
